    ...
    @if ($conversation)
        ...
        <script>
            (() => {
                ...
                /* CRM_CHAT_UI_PERFORMANCE_V1: serialize realtime message sync. */
                let messageSyncInFlight = null;
                let messageSyncPending = false;

                const pollMessages = () => {
                    messageSyncPending = true;

                    if (messageSyncInFlight) {
                        return messageSyncInFlight;
                    }

                    messageSyncInFlight = Promise.resolve().then(async () => {
                        try {
                            while (messageSyncPending) {
    ...
    <script>
        (() => {
            ...
            */
            ...
            /* CRM_CHAT_UI_PERFORMANCE_V1: presence is sent once by the global activity widget. */
            refreshSidebar();
            ...
            (() => {
                const bootChatNewestV13 = () => {
                    ...
                    const scheduleNewest = () => {
                        if (! followNewest || newestFrame !== null) {
                            return;
                        }

                        newestFrame = window.requestAnimationFrame(() => {
                            newestFrame = null;
                            goNewest();
                        });
                    };
                    ...
                    if ('ResizeObserver' in window) {
                        const newestResizeObserver = new ResizeObserver(scheduleNewest);
                        newestResizeObserver.observe(stack);
                        newestResizeObserver.observe(root);
                    }
                    ...
                    requestNewest();
                    root.addEventListener('load', scheduleNewest, true);
                    window.addEventListener('resize', scheduleNewest);
