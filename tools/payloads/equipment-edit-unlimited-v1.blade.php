...
<script>
    /* CRM_DELIVERY_ORDER_UNLIMITED_ITEMS_SCRIPT_V1 */
    (() => {
        const initializeEquipmentEditor = () => {
            const root = document.querySelector('[data-crm-equipment-editor]');
            ...
            const rowsContainer = root.querySelector('[data-equipment-rows]');
            const template = root.querySelector('[data-equipment-template]');
            const countLabel = root.querySelector('[data-equipment-count]');
            const searchInput = root.querySelector('[data-equipment-search]');
            const emptySearch = root.querySelector('[data-equipment-empty-search]');
            const tableWrap = root.querySelector('[data-equipment-table-wrap]');
            let nextIndex = Number.parseInt(root.dataset.nextIndex || '0', 10);

            const rows = () => Array.from(rowsContainer.querySelectorAll('[data-equipment-row]'));
            ...
            const refresh = () => {
                ...
                countLabel.textContent = `${allRows.length} item`;
                applySearch();
            };
                    ...
                    const values = Array.from(row.querySelectorAll('input, select'))
                        .map((field) => {
                            if (field.tagName === 'SELECT') {
                                const selectedText = field.options[field.selectedIndex]?.textContent || '';
                        ...
                        .join(' ')
                        .toLowerCase();
                    const matches = query === '' || values.includes(query);
            ...
            const addRow = () => {
                ...
                newRow?.querySelector('[data-equipment-name]')?.focus();
            ...
            root.addEventListener('click', (event) => {
                ...
                const row = removeButton.closest('[data-equipment-row]');
                const hasValue = Array.from(row.querySelectorAll('input[type="text"], select'))
                    .some((field) => String(field.value || '').trim() !== '');

                if (hasValue && ! window.confirm('Hapus item ini dari Surat Jalan?')) {
                    return;
                }
            ...
            root.addEventListener('change', (event) => {
                const select = event.target.closest('[data-inventory-select]');
                if (! select) return;

                const row = select.closest('[data-equipment-row]');
                const option = select.options[select.selectedIndex];
                const nameInput = row.querySelector('[data-equipment-name]');
                const unitInput = row.querySelector('[data-equipment-unit]');

                if (option?.value && nameInput && nameInput.value.trim() === '') {
                    nameInput.value = option.dataset.name || option.textContent.trim();
                }

                if (option?.value && unitInput && ['unit', ''].includes(unitInput.value.trim().toLowerCase())) {
                    unitInput.value = option.dataset.unit || 'unit';
                }
        ...
        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', initializeEquipmentEditor, { once: true });
        } else {
