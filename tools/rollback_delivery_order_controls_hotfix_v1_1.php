...
echo "ROLLBACK DELIVERY ORDER CONTROLS HOTFIX V1.1\n";
echo "=============================================\n\n";
...
$pattern = $root.DIRECTORY_SEPARATOR.'storage'.DIRECTORY_SEPARATOR.'app'.DIRECTORY_SEPARATOR.'private'
    .DIRECTORY_SEPARATOR.'patch-backups'.DIRECTORY_SEPARATOR.'delivery-order-controls-hotfix-v1-1-*';
$backups = glob($pattern, GLOB_ONLYDIR) ?: [];
rsort($backups);
$backup = $backups[0] ?? null;

if ($backup === null || ! is_file($backup.DIRECTORY_SEPARATOR.'manifest.json')) {
    fwrite(STDERR, "ROLLBACK GAGAL: Backup hotfix V1.1 tidak ditemukan.\n");
    exit(1);
}
...
foreach ($files as $relative => $metadata) {
    ...
    if (is_file($target) && ! unlink($target)) {
        fwrite(STDERR, "ROLLBACK GAGAL saat menghapus asset hotfix {$relative}.\n");
        exit(1);
    }
